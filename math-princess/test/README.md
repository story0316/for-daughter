...
test/
  ...
  unit/*.test.js         순수 로직(problems.js/subjects.js/endings.js/scenarios.js/
                         game-engine.js) 유닛 테스트 — 브라우저 불필요, plain Node로
                         즉시 실행됨, 아주 빠름
...
1. `problems.js`/`subjects.js`/`endings.js`/`scenarios.js`/`game-engine.js`처럼
   DOM에 의존하지 않는 순수 로직을 건드렸다면 `test/unit/`에 케이스를 추가하세요.
2. 골드/스탯 보상 공식이나 활동 종류(스케줄에 새 항목을 추가하는 등)를
   `game-engine.js`에서 바꿨다면 `test/balance/simulate.js`는 그 엔진을
   그대로 `require`해서 쓰므로 손으로 다시 맞출 필요가 없습니다. 다만 새
   활동/세션 유형을 추가했다면 시뮬레이터의 플레이어 정책(`runWeekActivity`
   등)에도 그 유형을 어떻게 다룰지 반영해줘야 합니다.
