...
test/
  ...
  unit/*.test.js         순수 로직(problems.js/subjects.js/endings.js/scenarios.js) 유닛 테스트
                         — 브라우저 불필요, plain Node로 즉시 실행됨, 아주 빠름
  balance/simulate.js    48개월 x 4주 생활 계획표 전체를 실제 공식으로 재현하는
...
1. `problems.js`/`subjects.js`/`endings.js`/`scenarios.js`처럼 DOM에 의존하지
   않는 순수 로직을 건드렸다면 `test/unit/`에 케이스를 추가하세요.
2. 골드/스탯 보상 공식이나 활동 종류(스케줄에 새 항목을 추가하는 등)를
   바꿨다면 `test/balance/simulate.js`도 함께 갱신해야 합니다(이 파일은
   script.js의 상수/공식을 그대로 옮겨 적어둔 것이라 원본이 바뀌면
   여기도 수동으로 맞춰야 정확합니다).
